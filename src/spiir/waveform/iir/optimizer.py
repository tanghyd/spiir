...
import numpy as np
from ._optimizer import calc_y_dy_ddy, calc_Y, calc_dY, calc_ddY, calc_ddYd
from ._optimizer import recalc_y_dy_ddy, recalc_Y, recalc_dY, recalc_ddY
from ._optimizer import chol_update, chol_downdate
...
def optimize_a1(
    ...
    if state is None:
        ...
        if verbose:
            print(f"Pass 0, overlap {overlap}", file=sys.stderr)
    else:
        ...
        if indv:
            ...
            while True:
                ...
                if np.max(np.abs(da)) < 1e-9:
                    ...
                    stop = True
        if verbose:
            print(f"Pass {i+1}, overlap {overlap}", file=sys.stderr)
        if stop:
            break
    if return_state:
